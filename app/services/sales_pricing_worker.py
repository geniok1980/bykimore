...
class SalesPricingWorker:
    ...
    async def _single_cycle(self) -> None:
        ...
        async with self._db_session_factory() as db:  # type: AsyncSession
            ...
            for dish in dishes_list:
                ...
                # Применение новой цены: сначала пытаемся отправить на iikoServer, затем фиксируем в нашей БД
                # TTL как расписание: следующий допуск к попытке изменения цены
                # ttl_min уже вычислен выше при расчёте dish_start
                next_allowed = (now + dt.timedelta(minutes=ttl_min)) if ttl_min > 0 else None

                if new_price is not None:
                    pushed = await self._push_price_to_iiko(dish, float(new_price))
                    if not pushed:
                        logger.error(
                            "Failed to push price to iikoServer for dish_id=%s name='%s' product_id=%s new_price=%.2f",
                            dish.id,
                            (dish.name or ""),
                            (dish.product_id or ""),
                            float(new_price),
                        )
                        # Не коммитим цену в локальную БД, чтобы не расходиться с iiko
                    else:
                        db.add(Price(dish_id=dish.id, value=float(new_price)))
                        # Рассчитываем курс как процент изменения относительно базовой цены
                        # Формула: ((new - base) / base) * 100, при base <= 0 курс = 0
                        try:
                            b = float(base_value or 0.0)
                            n = float(new_price)
                            if b <= 0:
                                rate_val = 0.0
                            else:
                                rate_val = ((n - b) / b) * 100.0
                        except Exception:
                            rate_val = 0.0
                        db.add(Rate(dish_id=dish.id, value=float(rate_val)))
                else:
                    pass
    ...
    async def _push_price_to_iiko(self, dish: Dish, new_price: float) -> bool:
        """Best-effort отправка новой цены в iikoServer.

        Синхронизировано с подходом из test_iiko_server_fetch.update_product_price:
        - основной формат: JSON с полем defaultSalePrice (число или объект {value})
        - добавляем fast code (num), генерируем если отсутствует, и передаём overrideFastCode=true
        - пытаемся как один объект и как массив из одного объекта
        - В случае 401/403 — выполняем реавторизацию и повторяем
        - Успех только если HTTP 200 и body.result == 'SUCCESS' (или отсутствуют ошибки)
        """
        try:
            if not getattr(dish, "product_id", None):
                try:
                    logger.error("Push aborted: missing product_id for dish_id=%s name='%s'", dish.id, (dish.name or ""))
                except Exception:
                    pass
                return False
            mgr = get_iiko_server_auth_manager()
            ...
            if base.lower().endswith("/resto"):
                base = base[:-len("/resto")]
            # Согласно сообщениям об ошибках ProductDto и проверкам, корректный путь:
            # /resto/api/v2/entities/products/update — принимает один объект ProductDto
            # /resto/api/v2/products/update на данном сервере отсутствует (404), поэтому исключаем его.
            update_urls = [
                f"{base}/resto/api/v2/entities/products/update",
            ]

            async def do_try_json() -> bool:
                ...
                async def fetch_product_details_by_id(pid: str) -> Optional[Dict[str, Any]]:
                    """Получить детальные поля продукта через /resto/api/v2/entities/products/list."""
                    if not session_key:
                        return None
                    list_urls = [
                        f"{base}/resto/api/v2/entities/products/list",
                        f"{base}/resto/api/v2/products/list",
                    ]
                    for list_url in list_urls:
                        try:
                            # GET c повторяющимися ids
                            params_list: List[tuple] = [("key", session_key), ("includeDeleted", "false"), ("revisionFrom", "-1"), ("ids", str(pid))]
                            r = await client.get(list_url, params=params_list, headers={"Accept": "application/json"}, timeout=30)
                            if r.status_code == 200:
                                ...
                                if isinstance(data, dict):
                                    for k in ("items", "products", "dishes", "list"):
                                        arr = data.get(k)
                                        if isinstance(arr, list) and arr:
                                            candidate_lists = arr
                                            break
                                elif isinstance(data, list):
                                    candidate_lists = data
                                if isinstance(candidate_lists, list):
                                    for it in candidate_lists:
                                        try:
                ...
                try:
                    details = await fetch_product_details_by_id(pid)
                except Exception:
                    details = None
                if isinstance(details, dict):
                    try:
                        body_base = dict(details)
                    except Exception:
                        body_base = {}
                    body_base["id"] = pid
                    if not str(body_base.get("num") or "").strip():
                        try:
                            seed = int(hashlib.sha1(pid.encode("utf-8")).hexdigest()[:8], 16)
                            body_base["num"] = 10000 + (seed % 90000)
                        except Exception:
                            pass
                    try:
                        name_val = str(body_base.get("name") or "").strip() or (dish.name or "").strip() or "Unnamed Product"
                        body_base["name"] = name_val
                    except Exception:
                        body_base["name"] = str(body_base.get("name") or "Unnamed Product")
                    parent_val = details.get("parent")
                    if isinstance(parent_val, dict):
                        parent_val = parent_val.get("id") or parent_val.get("guid") or parent_val.get("uuid") or parent_val.get("groupId")
                    if not (isinstance(parent_val, str) and parent_val.strip()):
                        pg = details.get("productGroup") or details.get("group") or details.get("parent")
                        if isinstance(pg, dict):
                            parent_val = pg.get("id") or pg.get("guid") or pg.get("uuid") or pg.get("groupId")
                        else:
                            parent_val = details.get("parentGroupId") or details.get("groupId") or details.get("productGroupId")
                    if not (isinstance(parent_val, str) and parent_val.strip()):
                        try:
                            parent_val = self._parent_cache.get(str(pid))
                        except Exception:
                            parent_val = parent_val
                    try:
                        if isinstance(parent_val, str) and parent_val.strip():
                            self._parent_cache[str(pid)] = parent_val.strip()
                    except Exception:
                        pass
                    try:
                        logger.warning(
                            "Push price: dish_id=%s pid=%s parent=%s",
                            dish.id,
                            pid,
                            (parent_val if parent_val is not None else "<null>")
                        )
                    except Exception:
                        pass
                    body_base["parent"] = (parent_val.strip() if isinstance(parent_val, str) and parent_val.strip() else None)
                    try:
                        self._last_push_parent[str(pid)] = body_base["parent"]
                    except Exception:
                        pass

                else:
                    # Если детали продукта не получены — прерываем пуш цены, так как обязаны
                    # явно передать текущий parent из ответа products/list
                    try:
                        logger.error("Push aborted: no product details for dish_id=%s pid=%s", dish.id, pid)
                    except Exception:
                        pass
                    return False
                ...
                # Не изменяем sizePrices/prices колонки вручную — отправляем то, что получили из GET

                # Основные варианты из test_iiko_server_fetch: defaultSalePrice (число или объект)
                # Корректные варианты поля цены для ProductDto: defaultSalePrice
                # Пробуем как число и как объект {value}
                # defaultSalePrice должен быть числом (BigDecimal). Вариант объекта вызывает 400 START_OBJECT.
                payloads: List[Dict[str, Any]] = [
                    {**body_base, "defaultSalePrice": float(new_price)},
                ]
                ...
                def is_success_response(resp: httpx.Response) -> bool:
                    ...
                    if isinstance(body, dict):
                        ...
                        return (result_val == "SUCCESS") or (isinstance(errors, list) and len(errors) == 0)
                    return False
                ...
                try:
                    for url in update_urls:
                        for payload in payloads:
                            try:
                                orig_keys = set(details.keys()) if isinstance(details, dict) else set()
                                pay_keys = set(payload.keys())
                                missing_keys = sorted(list(orig_keys - pay_keys))
                                extra_keys = sorted(list(pay_keys - orig_keys))
                                logger.warning(
                                    "PUSH JSON fields: orig=%d payload=%d missing=%s extra=%s",
                                    len(orig_keys),
                                    len(pay_keys),
                                    missing_keys,
                                    extra_keys,
                            ...
                            try:
                                logger.warning(
                                    "PUSH JSON -> url=%s parent=%s keys=%s",
                                    url,
                                    (body_base.get("parent") if body_base.get("parent") is not None else "<null>"),
                                    sorted(list(payload.keys()))
                                )
                            except Exception:
                                pass
                            # Попытка как один объект
                            resp = await client.post(url, params=params, headers=headers, json=payload, timeout=30)
                            try:
                                if resp.status_code >= 400:
                                    logger.error(
                                        "PUSH JSON <- status=%s body=%s",
                                        resp.status_code,
                            ...
                            if is_success_response(resp):
                                ...
                                try:
                                    logger.warning(
                                        "PUSH JSON <- success status=%s parent=%s result=%s errors=%s",
                                        resp.status_code,
                                        (body_base.get("parent") if body_base.get("parent") is not None else "<null>"),
                                    ...
                                    pass
                                return True
                            # На 401/403 — реавторизуемся и повторяем ту же попытку
                            if resp.status_code in (401, 403):
                                await mgr.reauthenticate()
                                resp2 = await client.post(url, params=params, headers=headers, json=payload, timeout=30)
                                try:
                                    if resp2.status_code >= 400:
                                        logger.error(
                                            "PUSH JSON (retry) <- status=%s body=%s",
                                            resp2.status_code,
                                            (resp2.text[:300] if resp2.text else "")
                                        )
                                except Exception:
                                    pass
                                if is_success_response(resp2):
                                    try:
                                        body2 = resp2.json()
                                    except Exception:
                                        body2 = None
                                    try:
                                        logger.warning(
                                            "PUSH JSON (retry) <- success status=%s parent=%s result=%s errors=%s",
                                            resp2.status_code,
                                            (body_base.get("parent") if body_base.get("parent") is not None else "<null>"),
                                            (body2.get("result") if isinstance(body2, dict) else None),
                                            (body2.get("errors") if isinstance(body2, dict) else None),
                                        )
                                    except Exception:
                                        pass
                                    return True

                            # Endpoint ожидает один объект ProductDto; массив вызвал бы START_ARRAY ошибку,
                            # поэтому попытки со списком исключаем.
                except Exception as e:
                    pass
                return False

            async def do_try_form() -> bool:
                ...
                data_variants = [
                    {"id": str(dish.product_id), "salePrice": str(new_price)},
                    {"id": str(dish.product_id), "price": str(new_price)},
                ]
                try:
                    for url in update_urls:
                        for data in data_variants:
                            try:
                                logger.error(
                                    "PUSH FORM -> %s params=%s data=%s",
                                    url,
                                    {"key_present": bool(params and params.get("key"))},
                                    data,
                                )
                            except Exception:
                                pass
                            resp = await client.post(url, params=params, headers=headers, data=data, timeout=30)
                            try:
                                logger.error(
                                    "PUSH FORM <- status=%s body=%s",
                                    resp.status_code,
                            ...
                            if resp.status_code in (401, 403):
                                await mgr.reauthenticate()
                                resp2 = await client.post(url, params=params, headers=headers, data=data, timeout=30)
                                try:
                                    logger.error(
                                        "PUSH FORM (retry) <- status=%s body=%s",
                                        resp2.status_code,
                                        (resp2.text[:200] if resp2.text else "")
                                    )
                                except Exception:
                                    pass
            ...
            # Пытаемся JSON (по аналогии с test_iiko_server_fetch). FORM оставляем как запасной вариант.
            if await do_try_json():
                return True
            # На этом сервере FORM-формат возвращал 415/404, отключаем чтобы не засорять логи.
            # if await do_try_form():
            #     return True

            # Можно добавить другие кандидаты эндпоинтов, если ваш сервер использует альтернативные пути
            logger.error("Failed to push price to iikoServer via entities/products/update for product_id=%s", (dish.product_id or ""))
            return False
        except Exception as e:
            logger.error("Push price to iikoServer error for product_id=%s: %s", (dish.product_id or ""), e)
            return False
